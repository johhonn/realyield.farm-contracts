...
function mint(address _to, uint256 _id, uint256 _amount, bytes memory _data)  isGame(msg.sender) {
     _mint(_to, _id,  _amount, _data);
}
function burn(address _from, uint256[] memory _ids, uint256[] memory _amounts)  isGame(msg.sender) returns(true){
